...
export default function Courses() {
  ...
  const filteredAndSortedCourses = useMemo(() => {
    ...
    switch (sortBy) {
      case "popular":
        courses = courses.sort((a, b) => b.studentsEnrolled - a.studentsEnrolled);
        break;
      case "rating":
        courses = courses.sort((a, b) => b.rating - a.rating);
        break;
      case "newest":
        courses = courses.sort((a, b) => new Date(b.createdDate).getTime() - new Date(a.createdDate).getTime());
        break;
      case "price-low":
    ...
    return courses;
  }, [searchQuery, selectedCategory, selectedLevel, selectedRating, priceRange, sortBy]);
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-start justify-between gap-4">
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-sm text-muted-foreground line-clamp-2 mb-2">{course.shortDescription}</p>
                ...
                <div className="text-right flex-shrink-0">
                  <div className="text-xl font-bold mb-1">{formatPrice(course.price)}</div>
                  {course.originalPrice && (
                      ...
                      <div className="space-y-2">
                        ...
                        {categories.map(category => (
                          <label key={category.id} className="flex items-center space-x-2">
                            ...
                            <span className="text-xs text-muted-foreground">({category.courseCount})</span>
                          </label>
                        ))}
                      </div>
                    </CardContent>
